...
class Ui_MainWindow(object):
    ...
    def start_video(self):
        if self.wedcam_radiobutton.isChecked():
            ...
            self.video2 = cv2.VideoCapture(2, cv2.CAP_DSHOW)

            self.set_cap(self.video0)
            self.set_cap(self.video1)
            self.set_cap(self.video2)
            ...
            while(self.video0.isOpened() and self.video1.isOpened() and self.video2.isOpened()):
                ...
                self.Procam.Top_cam(self.image0, self.process_l, self.process_r)
                self.Procam.Side_cam(self.image1)
                self.Procam.Front_cam(self.image2)
